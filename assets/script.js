const apiKey = "234238bf5e33ebe62a5d0060dfb5738a";
...
//Returning the current wether forecast
function returnCurrentWeather(cityName) {
    ...
    $.get(queryURL).then(function(response){
        ...
        currWeatherDiv.html(`
        <h2>${response.name} (${currTime.getMonth()+1}/${currTime.getDate()}/${currTime.getFullYear()})<img src=${weatherIcon} height="70px"></h2>
...
function returnWeatherForecast(cityName) {
    ...
    $.get(queryURL).then(function(response){
        ...
        $.each(forecastInfo, function(i) {
            if (!forecastInfo[i].dt_txt.includes("15:00:00")) {
                return;
            }
...
// Converting the UV Index to display the correct background colors
...
// Adding the abilty to click on the Cuty located in the history to display its results
